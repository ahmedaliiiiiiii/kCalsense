...
class NotificationHelper {
  ...
  static Future<void> init() async {
    ...
    // تنظيف الإشعارات القديمة (التي ليس لها eventType) - تحدث مرة واحدة فقط
    final prefs = await SharedPreferences.getInstance();
    final existing = prefs.getString('notifications_history');
    if (existing != null && existing.isNotEmpty) {
      try {
        final list = jsonDecode(existing) as List;
        if (list.isNotEmpty &&
            list.first is Map &&
            !(list.first as Map).containsKey('eventType')) {
          await prefs.remove('notifications_history');
          print('✅ Old notifications cleared – new events will be translated.');
        }
      } catch (_) {}
    }
  }

  /// عرض إشعار من حدث مع ترجمة فورية حسب اللغة المخزنة
  ...
  static Future<List<Map<String, dynamic>>> getTranslatedNotifications() async {
    ...
    for (var notif in rawNotifications) {
      if (notif.containsKey('title') &&
        ...
        // إشعار قديم (نصوص ثابتة) – نضيفه كما هو
        translated.add({
          'title': notif['title'],
          'body': notif['body'],
